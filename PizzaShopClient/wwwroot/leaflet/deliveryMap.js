(function () {
    ...
    export function animateMarkerMove(marker, coords, durationMs) {
    ...
    export function animateMarkerMoveFrame(marker) {
...
export function showOrUpdate1(elementId, markers) {
    var elem = document.getElementById(elementId);
    if (!elem) {
        throw new Error('No element with ID ' + elementId);
    }
    ...
    if (!elem.map) {
        elem.map = L.map(elementId);
        elem.map.addedMarkers = [];
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: 'Map data &copy; <a href="https://www.openstreetmap.org/">OpenStreetMap</a> contributors, <a href="https://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>' }).addTo(elem.map);
    }
